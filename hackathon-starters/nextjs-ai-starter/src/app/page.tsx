...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#f0fdf4_0,#f8fafc_260px)] text-zinc-900">
      <header className="border-b border-emerald-900/10 bg-white/90 backdrop-blur"><div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-xl bg-emerald-700 text-white"><WalletCards size={19}/></div><h1 className="text-lg font-bold tracking-tight text-emerald-950">TrustPay</h1></div><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-full bg-emerald-100 text-sm font-bold text-emerald-800">{user.name.charAt(0).toUpperCase()}</div><div className="hidden leading-tight sm:block"><p className="text-sm font-semibold">{user.name}</p><p className="max-w-52 truncate text-xs text-zinc-500">{user.email}</p></div><button className="h-9 rounded-lg border border-zinc-200 bg-white px-3 text-xs font-semibold text-zinc-700 hover:bg-zinc-50" onClick={async () => { await api("/auth/logout", { method: "POST" }); setUser(null); }}>Log out</button></div></div></header>

      <div className="mx-auto max-w-7xl space-y-5 px-5 py-6">
        <section className="rounded-3xl bg-gradient-to-br from-emerald-700 via-emerald-800 to-emerald-950 p-6 text-white shadow-xl shadow-emerald-950/15"><div><p className="text-sm font-medium text-emerald-100">Available Balance</p><p className="mt-1 text-4xl font-bold tracking-tight">{formatTaka(balance)}</p><p className="mt-2 text-sm text-emerald-200">Your TrustPay wallet</p></div><div className="mt-6 grid grid-cols-3 gap-2"><button className="rounded-2xl bg-white/10 p-3 text-center text-xs font-semibold transition hover:bg-white/20" onClick={() => document.getElementById("send-money")?.scrollIntoView({ behavior: "smooth" })}><ArrowUpRight className="mx-auto mb-1" size={20}/>Send Money</button><button className="rounded-2xl bg-white/10 p-3 text-center text-xs font-semibold transition hover:bg-white/20" onClick={() => document.getElementById("request-money")?.scrollIntoView({ behavior: "smooth" })}><ArrowDownLeft className="mx-auto mb-1" size={20}/>Request Money</button><button className="rounded-2xl bg-white p-3 text-center text-xs font-semibold text-emerald-900 transition hover:bg-emerald-50" onClick={() => { setPayMode("intent"); document.getElementById("quick-pay")?.scrollIntoView({ behavior: "smooth" }); }}><Sparkles className="mx-auto mb-1" size={20}/>Smart Quick Pay</button></div></section>

        {(notice || error) && <div className={`rounded-xl border px-4 py-3 text-sm ${error ? "border-red-200 bg-red-50 text-red-700" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}>{error || notice}</div>}

        {scheduled && <section className="fixed inset-x-4 bottom-4 z-40 mx-auto flex max-w-2xl items-center justify-between gap-4 rounded-2xl border border-amber-300 bg-amber-50 p-4 shadow-2xl"><div><p className="font-semibold text-amber-950">Sending {formatTaka(scheduled.draft.amountTaka * 100)} to {scheduled.draft.recipientName} in {scheduled.seconds} seconds</p><p className="text-sm text-amber-800">You can cancel before the countdown ends.</p></div><button className="h-11 shrink-0 rounded-xl bg-red-600 px-5 text-sm font-bold text-white" onClick={undoPayment}>UNDO</button></section>}
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <section id="send-money" className={cardClass}><SectionTitle title="Send Money" subtitle="Pay a TrustPay account by email"/><form className="mt-4 space-y-3" onSubmit={(event) => { event.preventDefault(); void reviewPayment({ recipientName: manual.recipientEmail, recipientEmail: manual.recipientEmail, amountTaka: Number(manual.amountTaka), note: manual.note }, "manual"); }}><Field label="Recipient"><input className={inputClass} type="email" required placeholder="recipient@example.com" value={manual.recipientEmail} onChange={(event) => { setManual({ ...manual, recipientEmail: event.target.value }); setManualError(""); }}/>{manualError && <p className="mt-1.5 text-sm text-red-600">{manualError}</p>}</Field><Field label="Amount (Taka)"><input className={inputClass} type="number" min="0.01" step="0.01" required placeholder="500" value={manual.amountTaka} onChange={(event) => setManual({ ...manual, amountTaka: event.target.value })}/></Field><Field label="Note (optional)"><input className={inputClass} maxLength={200} placeholder="Lunch" value={manual.note} onChange={(event) => setManual({ ...manual, note: event.target.value })}/></Field><button className={`${primaryButton} w-full`} disabled={isReviewingPayment || Boolean(scheduled)}>{isReviewingPayment ? "Checking..." : "Review Payment"}</button></form></section>
          <section id="request-money" className={cardClass}><SectionTitle title="Request Money" subtitle="Ask another TrustPay user to pay you"/><form className="mt-4 space-y-3" onSubmit={submitRequest}><Field label="Payer"><input className={inputClass} type="email" required placeholder="payer@example.com" value={moneyRequest.payerEmail} onChange={(event) => { setMoneyRequest({ ...moneyRequest, payerEmail: event.target.value }); setRequestError(""); }}/>{requestError && <p className="mt-1.5 text-sm text-red-600">{requestError}</p>}</Field><Field label="Amount (Taka)"><input className={inputClass} type="number" min="0.01" step="0.01" required placeholder="500" value={moneyRequest.amountTaka} onChange={(event) => setMoneyRequest({ ...moneyRequest, amountTaka: event.target.value })}/></Field><Field label="Note (optional)"><input className={inputClass} maxLength={200} placeholder="Lunch" value={moneyRequest.note} onChange={(event) => setMoneyRequest({ ...moneyRequest, note: event.target.value })}/></Field><button className={`${primaryButton} w-full`} disabled={isSendingRequest}>{isSendingRequest ? "Sending request..." : "Send Request"}</button></form></section>
        </div>

        <section className={cardClass}><SectionTitle title="Money Requests" subtitle="Incoming and outgoing payment requests"/><div className="mt-4 grid gap-5 lg:grid-cols-2"><RequestList title="Incoming" items={incoming} incoming actions={updateRequest}/><RequestList title="Outgoing" items={outgoing}/></div></section>
        <section className={cardClass}><SectionTitle title="Recent Transactions" subtitle="Your latest sent and received payments"/><div className="mt-3 divide-y divide-zinc-100">{history.slice(0, 10).map((item) => <div key={item.id} className="flex items-center gap-3 py-4"><div className={`grid size-10 shrink-0 place-items-center rounded-full ${item.type === "SENT" ? "bg-zinc-100 text-zinc-600" : "bg-emerald-100 text-emerald-700"}`}>{item.type === "SENT" ? <ArrowUpRight size={18}/> : <ArrowDownLeft size={18}/>}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{item.type === "SENT" ? "Sent to" : "Received from"} {item.person.name}</p><p className="truncate text-xs text-zinc-500">{item.note || item.person.email} · {new Date(item.createdAt).toLocaleString()}</p></div><div className="text-right"><p className={`text-sm font-bold ${item.type === "RECEIVED" ? "text-emerald-700" : ""}`}>{item.type === "RECEIVED" ? "+" : "−"}{formatTaka(item.amountPoysha)}</p><p className="text-xs font-medium text-zinc-500">{item.status}</p></div></div>)}{history.length === 0 && <p className="py-8 text-center text-sm text-zinc-500">No transactions yet</p>}</div></section>
      </div>

      {review && <div className="fixed inset-0 z-30 grid place-items-center bg-zinc-950/40 p-4 backdrop-blur-sm"><div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl"><p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">{review.phase === "security" ? "Trust Check" : "Payment Draft"}</p><h2 className="mt-1 text-xl font-bold">{review.phase === "security" ? review.risk.riskLevel : `Send ${formatTaka(review.draft.amountTaka * 100)} to ${review.draft.recipientName}?`}</h2>{review.phase === "security" && <p className="mt-2 text-sm text-zinc-500">TrustPay checks transaction context before money moves. Gemini does not make security decisions.</p>}<div className="my-5 rounded-2xl bg-zinc-50 p-4"><p className="font-semibold">{review.draft.recipientName}</p><p className="text-sm text-zinc-500">{review.draft.recipientEmail}</p><p className="mt-4 text-3xl font-bold">{formatTaka(review.draft.amountTaka * 100)}</p>{review.draft.note && <p className="mt-1 text-sm text-zinc-500">Note: {review.draft.note}</p>}</div>{review.phase === "security" && review.risk.reasons.length > 0 && <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><p className="font-semibold">Why this needs attention</p>{review.risk.reasons.map((reason) => <p className="mt-1" key={reason}>• {reason}</p>)}</div>}<div className="grid grid-cols-2 gap-3"><button className={secondaryButton} onClick={() => setReview(null)}>Cancel</button>{review.phase === "security" ? <button className={primaryButton} onClick={() => setReview({ ...review, phase: "confirm" })}>{review.risk.requiresReview ? "Continue Anyway" : "Continue"}</button> : <button className={primaryButton} onClick={confirmPayment}>Confirm Payment</button>}</div></div></div>}
      {paymentSuccess && <div className="fixed inset-0 z-50 grid place-items-center bg-zinc-950/40 p-4 backdrop-blur-sm"><div className="w-full max-w-sm rounded-3xl bg-white p-6 text-center shadow-2xl"><div className="mx-auto grid size-14 place-items-center rounded-full bg-emerald-100 text-2xl font-bold text-emerald-700">✓</div><h2 className="mt-4 text-xl font-bold">Payment Successful</h2><p className="mt-2 text-3xl font-bold text-emerald-800">{formatTaka(paymentSuccess.draft.amountTaka * 100)}</p><p className="mt-3 text-sm text-zinc-500">Sent to</p><p className="font-semibold">{paymentSuccess.draft.recipientName}</p><div className="my-5 rounded-2xl bg-zinc-50 p-4 text-sm"><span className="text-zinc-500">New balance</span><strong className="ml-2 text-emerald-800">{formatTaka(paymentSuccess.newBalancePoysha)}</strong></div><button className={`${primaryButton} w-full`} onClick={() => setPaymentSuccess(null)}>Done</button></div></div>}
    </main>
  );
}
...
function AuthCard({ authMode, setAuthMode, registerStep, setRegisterStep, login, setLogin, registration, setRegistration, submitLogin, requestOtp, verifyOtp, notice, error }: any) {
  return <main className="grid min-h-screen place-items-center bg-[linear-gradient(145deg,#ecfdf5,#f8fafc)] p-6"><div className="w-full max-w-sm rounded-3xl border border-emerald-900/10 bg-white p-8 shadow-xl shadow-emerald-950/5"><div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-xl bg-emerald-700 text-white"><WalletCards/></div><div><h1 className="text-2xl font-bold text-emerald-950">TrustPay</h1><p className="text-xs text-zinc-500">Secure simulated payments</p></div></div><div className="my-6 grid grid-cols-2 rounded-xl bg-zinc-100 p-1"><button className={`rounded-lg py-2 text-sm ${authMode === "login" ? "bg-white font-semibold shadow-sm" : ""}`} onClick={() => setAuthMode("login")}>Login</button><button className={`rounded-lg py-2 text-sm ${authMode === "register" ? "bg-white font-semibold shadow-sm" : ""}`} onClick={() => setAuthMode("register")}>Register</button></div>{authMode === "login" ? <form className="space-y-4" onSubmit={submitLogin}><Field label="Email"><input className={inputClass} type="email" required value={login.email} onChange={(event) => setLogin({ ...login, email: event.target.value })}/></Field><Field label="Password"><input className={inputClass} type="password" required value={login.password} onChange={(event) => setLogin({ ...login, password: event.target.value })}/></Field><button className={`${primaryButton} w-full`}>Login</button></form> : registerStep === 1 ? <form className="space-y-4" onSubmit={requestOtp}><Field label="Name"><input className={inputClass} required value={registration.name} onChange={(event) => setRegistration({ ...registration, name: event.target.value })}/></Field><Field label="Email"><input className={inputClass} type="email" required value={registration.email} onChange={(event) => setRegistration({ ...registration, email: event.target.value })}/></Field><Field label="Password"><input className={inputClass} type="password" minLength={8} required value={registration.password} onChange={(event) => setRegistration({ ...registration, password: event.target.value })}/></Field><button className={`${primaryButton} w-full`}>Send OTP</button></form> : <form className="space-y-4" onSubmit={verifyOtp}><p className="text-sm text-zinc-600">Verification code sent to<br/><b>{registration.email}</b></p><Field label="6-digit OTP"><input className={`${inputClass} text-center text-lg tracking-[0.4em]`} inputMode="numeric" pattern="[0-9]{6}" maxLength={6} required value={registration.otp} onChange={(event) => setRegistration({ ...registration, otp: event.target.value.replace(/\D/g, "") })}/></Field><button className={`${primaryButton} w-full`}>Verify & Create Account</button><button type="button" className="w-full text-sm font-semibold text-emerald-700" onClick={() => void requestOtp()}>Resend OTP</button><button type="button" className="w-full text-sm text-zinc-500" onClick={() => setRegisterStep(1)}>Back</button></form>}{(notice || error) && <p className={`mt-4 rounded-lg p-3 text-sm ${error ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-800"}`}>{error || notice}</p>}</div></main>;
}
